...
Add `StellarToolsAppBootstrap` to your root layout. It injects a script that automatically attaches `st_token` as a `Bearer` header on every outgoing `fetch` call — you never thread the token through components manually.
...
<Note>
  The fetcher can be a server action (as above) or a plain `fetch` to one of your API routes. `StellarToolsAppBootstrap` attaches the auth header on every fetch automatically.
</Note>
