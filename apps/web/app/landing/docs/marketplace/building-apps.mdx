...
`st_token` is a signed JWT containing the org context. Because your app secret must stay server-side, verification happens in a server action or API route.
...
**Client provider** — reads the token from the URL, calls the server action, and mounts the context:
...
```env
YOUR_APP_SECRET=sec_...         # Your app's signing secret — used by resolveAppContext
