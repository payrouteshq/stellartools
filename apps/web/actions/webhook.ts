...
export const postWebhookLog = async (
 filter: { webhookId?: string } | { appInstallationId?: string}  ,
  ...
  const [webhookLog] = await db
    .insert(webhookLogs)
    .values({ ...params, ...('webhookId' in filter && { webhookId: filter.webhookId}), ...('appInstallationId' in filter && { appInstallationId: filter.appInstallationId}), organizationId, environment } as WebhookLog)
    .returning();
