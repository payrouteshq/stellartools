...
export default async function initial_data_seed({
  ...
  if (existingRegions.length > 0) {
    ...
    logger.info(`Region already exists (${region.name}), skipping creation.`);
  } else {
  ...
  // Skip tax region creation if they already exist
  ...
  const existingLocations = await query.graph({ entity: "stock_location", fields: ["id", "name"] });
  let stockLocation: any;
  if (existingLocations.data.length > 0) {
    stockLocation = existingLocations.data[0];
    logger.info(`Stock location already exists (${stockLocation.name}), skipping.`);
  } else {
  ...
  const existingShippingOptions = await query.graph({ entity: "shipping_option", fields: ["id"] });
  ...
  const existingProducts = await query.graph({ entity: "product", fields: ["id"] });
  if (existingProducts.data.length > 0) {
    logger.info("Products already exist, skipping product data seeding.");
