...
You need an API key from [dashboard.stellartools.dev/api-keys](https://dashboard.stellartools.dev/api-keys). The key sets your environment (testnet or mainnet), same as the REST API.
...
The server uses **SSE transport**: the client opens a long-lived `GET` stream, then sends JSON-RPC messages over `POST` with a `sessionId` query parameter. You do not configure that yourself — MCP clients handle it.

<Note>
  While building locally, use your API host (for example `http://api.localhost:3000/mcp`) if you run the app on a custom subdomain.
</Note>
