...
On the Add Plugins screen, click **Upload Plugin**, select your `stellartools.zip` file, then click **Install Now**.
...
<img src="/images/paymentoption.png" alt="WooCommerce Payments tab with StellarTools active and Manage button highlighted" />
...
<ParamField body="API Key" type="string" required>
  Your StellarTools secret key. Test keys begin with `sk_test_`; live keys begin with `sk_live_`. Find yours in the [StellarTools dashboard](https://dashboard.stellartools.dev/api-keys).
</ParamField>
...
<ParamField body="Webhook Signing Secret" type="string" required>
  Copy the signing secret from your webhook endpoint in the [StellarTools dashboard](https://dashboard.stellartools.dev/webhooks). Your webhook URL is shown inline directly in the settings page copy it from there.
</ParamField>

<ParamField body="Payment Asset" type="string">
